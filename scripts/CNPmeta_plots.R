...
ind_pft_results <- read.csv("../tables/CNPmeta_pft_moderators.csv")
int_pft_results <- read.csv("../tables/CNPmeta_clim_moderators_int.csv")
...
chemistry_ind_plot <- ggplot(data = meta_ci %>% 
  ...
  geom_text(data = subset(meta_ci_int, var %in% c("lma", 
                                                  ...
                                                  "leaf_p_area",
                                                  "leaf_np")),
            aes(label = k_plot, y = 1.1), size = 6, fontface = "bold") +
...
chemistry_int_plot <- ggplot(data = meta_ci_int %>% 
  ...
  scale_fill_manual(values = c("black", "purple", "green3")) +
  scale_x_discrete(labels = c("Leaf N:P",
                              expression(italic("P")["area"]), 
                              expression(italic("P")["mass"]), 
                              expression(italic("N")["area"]),
...
bio_int_plot <- ggplot(data = meta_ci_int %>% 
  ...
  geom_point(aes(fill = int_type), size = 4, shape = 21) +
  geom_hline(yintercept = 0, linewidth = 0.5, linetype = "dashed") +
  geom_text(aes(label = k_plot, y = 0.95), size = 6, fontface = "bold") +
  scale_fill_manual(values = c("black", "green3")) +
  scale_x_discrete(labels = c("Root:shoot",
                              "Root mass fraction",
                              "Belowground prod.",
...
leafnp_int_tg_plot <- mod_results(int_leafnp_clim, mod = "gs_mat",
  ...
  scale_size_continuous(limits = c(0, 224), range = c(1, 7)) +
  labs(title = expression(bold("Leaf N:P - int. resp. to ")*bolditalic("T")[bold("g")]),
...
leafnp_int_ai_plot <- ggplot() +
  ...
  scale_y_continuous(limits = c(-1.4, 1.4), breaks = seq(-1.4, 1.4, 0.7)) +
  scale_size_continuous(limits = c(0, 224), range = c(1, 7)) +
  labs(title = expression(bold("Leaf N:P - int. resp. to ")*bolditalic("MI")[bold("g")]),
...
leafnp_int_par_plot <- mod_results(int_leafnp_clim, mod = "gs_par",
  ...
  scale_y_continuous(limits = c(-1.4, 1.4), breaks = seq(-1.4, 1.4, 0.7)) +
  scale_size_continuous(limits = c(0, 224), range = c(1, 7)) +
  labs(title = expression(bold("Leaf N:P - int. resp. to ")*bolditalic("PAR")[bold("g")]),
       x = expression(bolditalic("MI")[bold("g")]*bold(" (unitless)")),
